...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_images(self, images):
        ...
        # --- DTYPE ALIGNMENT BEFORE mm_projector ---
        import torch  # make sure torch is imported at the top of the file

        proj = self.get_model().mm_projector

        # Get projector weight to infer its dtype
        if isinstance(proj, torch.nn.Sequential):
            proj_weight = proj[0].weight
        else:
            proj_weight = getattr(proj, "weight", None)

        if proj_weight is not None:
            target_dtype = proj_weight.dtype
            if isinstance(image_features, list):
                image_features = [f.to(dtype=target_dtype) for f in image_features]
            else:
                image_features = image_features.to(dtype=target_dtype)

        image_features = proj(image_features)
        # --- END DTYPE ALIGNMENT ---
        #B_tiles, T, D = image_features.shape
        #image_features = image_features.view(1, B_tiles * T, D)  # [1, 900, D]
